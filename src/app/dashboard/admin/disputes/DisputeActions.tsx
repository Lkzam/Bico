...
export function DisputeActions({ jobId, hasDelivery }: { jobId: string; hasDelivery: boolean }) {
  ...
  async function resolve(action: 'release' | 'refund') {
    ...
    if (!confirm(`Confirma ${label} este trabalho?`)) return

    setActing(action)
    const res = await fetch(`/api/admin/disputes/${jobId}/resolve`, {
      method: 'POST',
    ...
    if (!res.ok) {
      toast.error(json.error ?? 'Erro ao resolver disputa.')
      return
    }
    if (json.alreadyResolved) {
      toast.info('Esta disputa já tinha sido resolvida.')
    } else {
      toast.success(action === 'release' ? 'Pagamento liberado.' : 'Reembolso aprovado.')
    }
    router.refresh()
  }
  ...
  return (
    <div>
      {/* Nota interna do admin (vai pro log de auditoria) */}
      ...
      <div className="dash-btn-group" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: 6,
            padding: '9px 16px',
            background: acting ? 'rgba(34,197,94,0.25)' : 'rgba(34,197,94,0.15)',
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: 6,
            padding: '9px 16px',
            background: acting ? 'rgba(239,68,68,0.25)' : 'rgba(239,68,68,0.12)',
      ...
      <p style={{ fontSize: 10, color: 'rgba(185,190,200,0.3)', marginTop: 12, margin: 0 }}>
        ⚠️ "Reembolsar empresa" só marca a disputa como reembolsada no sistema. A devolução do PIX precisa ser processada manualmente no painel da Efí.
      </p>
    </div>
  )
}
