    ...
    <div style={{ minHeight: '100vh', background: '#0b0e17', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', fontFamily: 'var(--font-body), Inter, sans-serif' }}>
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        <h1 style={{
          ...
          Quase lá!<br />
          <span style={{ color: 'rgba(160,152,148,0.88)' }}>conta seu perfil</span>
        </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 32 }}>
          {([
            ...
            <button key={opt.value} type="button" onClick={() => setRole(opt.value)} style={{
              ...
              <div style={{ fontSize: 11, color: role === opt.value ? '#d4783a' : 'rgba(185,190,200,0.4)' }}>
                {opt.sub}
              </div>
            </button>
          ))}
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <button
                ...
                onMouseOut={e => { (e.currentTarget as HTMLButtonElement).style.background = loading ? 'rgba(217,78,24,0.5)' : '#d94e18' }}
              >
                {loading ? 'Salvando...' : 'Entrar no Bico'}
              </button>
            </div>
          </form>
        </div>
