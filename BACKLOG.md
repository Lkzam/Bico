...
### [x] Forçar `rating` a ser inteiro de 1 a 5 em avaliações
**Concluído em:** 2026-06-30
**Branch:** `agent/rating-zod-validation`
**PR:** https://github.com/Lkzam/Bico/pull/new/agent/rating-zod-validation *(gh CLI não instalado — abrir PR via interface web)*
**O que foi feito:**
- `src/app/api/reviews/submit/route.ts` agora usa `parseBody` + `reviewSubmitSchema` (zod): `jobArchiveId` = uuid, `rating` = inteiro 1–5, `comment` = string opcional máx 2000.
- `src/lib/__tests__/reviewSubmitSchema.test.ts` criado com 9 testes (rejeita `"abc"`, `3.7`, `0`, `6`; aceita `1`, `5`).
- `npm test`: 49/49 testes verdes. Lint: sem erros novos (erros pré-existentes em outros arquivos).
